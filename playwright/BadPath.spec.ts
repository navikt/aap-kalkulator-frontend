...
test("BadPath", async ({ page }) => {
    ...
    await expect(
        page.getByRole("heading", { name: "Vær oppmerksom på dette" })
    ).toBeVisible()
    ...
    await expect(
        page.getByText("Velg enten ja eller nei for å gå videre.")
    ).toBeVisible()
    await page
        .getByRole("combobox", {
    ...
    await page.getByRole("button", { name: "Gå videre" }).click()
    await expect(
        page.getByText("Du må velge enten ja eller nei for å gå videre.")
    ).toBeVisible()
    ...
    //Resultat side
    await expect(
        page.getByRole("heading", { name: "Det ser ut som du kan få" })
    ).toBeVisible()
})
