...
export const overArbeidsGrense = (arbeidsgrad: number, harAAP: boolean) => {
  return (arbeidsgrad!! > ARBEIDSGRENSEUTENAAP && !harAAP) || arbeidsgrad!! > ARBEIDSGRENSEMEDAAP;
};
...
export const arbeidsgrad = (resultat: Result) => {
  ...
  if (jobberIkke(resultat.personInfo!.arbeidsgrad!, resultat.personInfo!.harArbeid!)) {
    resultat.logs.push({ id: 'logic.work.noWork' });
    resultat.logs.push({ id: 'logic.work.onAll' });
    return;
  }

  if (overArbeidsGrense(resultat.personInfo!.arbeidsgrad!, resultat.personInfo!.harAAP!)) {
    resultat.resultat = 0.0;
    resultat.logs = [];
    resultat.logs.push({
      id: resultat.personInfo?.harAAP ? 'logic.work.tooMuchWithAAP' : 'logic.work.tooMuchWithoutAAP',
      values: {
